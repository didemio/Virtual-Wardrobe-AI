...
**Input:** Wardrobe with 4 items across 3 categories; query category `"tops"`
**Expected:** Returns exactly 2 items, both with `category == "tops"`
**Result:** ✅ Pass

### Scenario 2 – Filter is case-insensitive
**Function:** `filter_by_category`
**Input:** Query with uppercase `"TOPS"`
**Expected:** Same 2 results as lowercase query
**Result:** ✅ Pass

### Scenario 3 – Filter returns empty list for unknown category
**Function:** `filter_by_category`
**Input:** Category `"shoes"` (not in wardrobe)
**Expected:** Empty list `[]`
**Result:** ✅ Pass

### Scenario 4 – List categories returns unique values
**Function:** `list_categories`
**Input:** Wardrobe with items in `"outerwear"`, `"tops"`, `"bottoms"` (some repeated)
...
**Function:** `add_item`
**Input:** Add `"Shirt"` to `"tops"` in a wardrobe with 4 items
...
**Function:** `add_item`
**Input:** Add `"Baseball Cap"` under `"accessories"`
**Expected:** Last item has `name == "Baseball Cap"` and `category == "accessories"`
**Result:** ✅ Pass
...
- `ensure_ascii=False` is used throughout to correctly handle non-ASCII characters in item names.
